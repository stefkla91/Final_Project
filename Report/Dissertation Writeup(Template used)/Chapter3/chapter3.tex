...
In order to try to fix the localisation error, different approaches were taken, which will be described in the following sections. 

\section{Deployment algorithm refactoring and improvement}
...
\section{Selective wall following approach}
The deployment pattern as it was at this stage of the project had some major mapping errors. FIXME(find screenshots of this).\\
This approach used a boolean value which was set to true when the robot turns to follow the \textit{north} and \textit{east} wall, figure \ref{directions_and_angles} on page \pageref{directions_and_angles} shows  which walls are meant.\\
The reason behind this is to have the functionality to force the robot into the \textit{East to West} or the \textit{North to South} movement pattern respectively. 
